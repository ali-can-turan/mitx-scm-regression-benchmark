...
[See the full project](sca_ML.pdf)
...
| Model | Pipeline Architecture | Train (CV) $R^2$ | Validation $R^2$ | Train $R^2$ | Test $R^2$ | Test Adj. $R^2$ | Relative Bias (NMB) |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **ML-1 (Winner)** | **Business Logic + Unscaled Features** | **0.84** | **0.80** | **0.83** | **0.72** | **0.71** | **~1.8%** |
| ML-2 | Scaled + Poly(d=3) + `LassoCV` | 0.84 | 0.80 | 0.87 | 0.62 | 0.60 | ~2.8% |

*Note: Models were evaluated on a 90/10 split with 5-fold cross-validation during training.*

*Note: From ML-2, the results of the pipe with the best validation score are presented above. For the full results, including the alternative with less polynomial degree and unscaling, please refer to the notebook [here](sca_ML.pdf) or [here](sca_ML.ipynb).*  
...
> **Key Discovery:** Unconstrained hyperparameter tuning resulted in severe overfitting—causing a **15% performance drop** in Test Adjusted $R^2$ (falling from **0.71** in ML-1 down to **0.60** in ML-2).
